...
export default class MapEditor {
  ...
  _initFloor() {
    ...
    if (typeof (this.map.floorData.geometry) !== "undefined") {
      const v = (new ol.format.GeoJSON()).readFeatures(this.map.floorData);
      v.forEach((f) => {
    ...
    if (JSON.stringify(this.map.layerData.point) !== "{}") {
      const fs = (new ol.format.GeoJSON()).readFeatures(this.map.layerData.point);
      if (fs.length > 0) {
        fs.forEach((f) => {
    ...
    if (JSON.stringify(this.map.layerData.path) !== "{}") {
      const fs = (new ol.format.GeoJSON()).readFeatures(this.map.layerData.path);
      if (fs.length > 0) {
        fs.forEach((f) => {
    ...
    if (JSON.stringify(this.map.layerData.polygon) !== "{}") {
      const fs = (new ol.format.GeoJSON()).readFeatures(this.map.layerData.polygon);
      if (fs.length > 0) {
        fs.forEach((f) => {
  ...
  setBuildStyle(id, style) {
    const f = this.ol.layers.buildLayer.getSource().getFeatureById(id);
  ...
  setPointStyle(id, style) {
    const f = this.ol.layers.pointLayer.getSource().getFeatureById(id);
    if (typeof (f) !== "undefined") {
      this.interactionManage.clearSelectFeatures();
  ...
  setPathStyle(id, style) {
    const f = this.ol.layers.pathLayer.getSource().getFeatureById(id);
    if (typeof (f) !== "undefined") {
      this.interactionManage.clearSelectFeatures();
  ...
  setPolygonStyle(id, style) {
    const f = this.ol.layers.polygonLayer.getSource().getFeatureById(id);
    if (typeof (f) !== "undefined") {
      this.interactionManage.clearSelectFeatures();
