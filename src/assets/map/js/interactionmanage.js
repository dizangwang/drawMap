...
export class InterCtionManage {
  constructor(mapedit) {
    ...
    this.mapEditor.ol.interactions.select = new ol.interaction.Select({
      // /选择控件筛选器
      filter: (f, l) => {
        if (l == this.mapEditor.ol.layers.buildLayer && this._filter.build) return true;
        if (l == this.mapEditor.ol.layers.pointLayer && this._filter.point) return true;
        if (l == this.mapEditor.ol.layers.pathLayer && this._filter.path) return true;
        if (l == this.mapEditor.ol.layers.polygonLayer && this._filter.polygon) return true;
        return false;
      }
    });
    ...
    this.mapEditor.ol.interactions.select.on("select", (e) => {
      ...
      if (this.isRotate) this._endRotate();
      if (e.selected.length > 0) {
        if (typeof (this.mapEditor.event.selectFeature) !== "undefined") {
          const f = e.selected[0].getProperties();
          let layername = "";
          const l = this.mapEditor.ol.interactions.select.getLayer(e.selected[0]);
          if (l == this.mapEditor.ol.layers.buildLayer) layername = "建筑物图层";
          if (l == this.mapEditor.ol.layers.pointLayer) layername = "POI图层";
          if (l == this.mapEditor.ol.layers.pathLayer) layername = "路径图层";
          if (l == this.mapEditor.ol.layers.polygonLayer) layername = "多边形图层";
  ...
  _drawEnd(f, layer, s = null) {
    ...
    if (layer == "point") {
      ...
      if (s != null) style = Style.objToPointStyle(s);
    ...
    if (layer == "path") {
      ...
      if (s != null) style = Style.objToPathStyle(s);
    ...
    if (layer == "polygon") {
      ...
      if (s != null) style = Style.objToPolygonStyle(s);
  ...
  // /删除选择要素
  removeSelectFeature() {
    if (this.mapEditor.ol.interactions.select.getFeatures().getLength() == 0) return;
  ...
  _removeInteraction(interaction) {
    const interactions = this.mapEditor.ol.map.getInteractions();
    interactions.forEach((e) => {
      if (e && interaction && e.ol_uid && interaction.ol_uid && typeof (e.ol_uid) !== "undefined" && typeof (interaction.ol_uid) !== "undefined") if (e.ol_uid == interaction.ol_uid) this.mapEditor.ol.map.removeInteraction(interaction);
    });
  }
  ...
  _onRotateMapDrag(event) {
    ...
    this.getSelectFeatures().forEach((f) => {
      ...
      const c = ol.extent.getCenter(g.getExtent());
      const a = getAngle({
